...
I am going to pick my simplest endpoint which checks if the logged-in user is friends with the supplied userId with path `/friendships/{userId}`.  In this Java Test class I use MockWebServer to mock responses for Rest callouts which is not used in this particular test.  The following is my Java code:
