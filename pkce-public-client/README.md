...
The client authentication method is done using `client_secret_basic` where means that the client id is sent in the http header similar to basic authentication http header.  We also check `NONE` because we are using PKCE and we are going to be sending a `code_verifier` and a `code_challenge` pairs in our exchanges with the authorization server for receiving a token.  For more on this `NONE` read [this](https://cloudentity.com/developers/basics/oauth-client-authentication/client-auth-set-to-none-with-pkce/).
